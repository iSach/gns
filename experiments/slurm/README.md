...
`$GNS_RUNS`, which defaults to `runs/` inside the repository. Point it at
scratch or scale storage instead: checkpoints are 19 MB each.
...
| `GNS_RUNS` | `$GNS_REPO/runs` | Where run directories go. Put it on scratch, not in the repo. |
| `GNS_DATA_ROOT` | required | Where the converted datasets are. |
